...
void bestFit(struct MemoryBlock memory[], int numBlocks, int processSize) {
    int bestFitIdx = -1;
    int smallestBlockSize = MAX_MEMORY_SIZE;

    // Find the smallest available block of memory that fits the process
    for (int i = 0; i < numBlocks; i++) {
        if (memory[i].size >= processSize && memory[i].size < smallestBlockSize) {
            bestFitIdx = i;
    ...
    if (bestFitIdx != -1) {
        memory[bestFitIdx].size -= processSize;
        printf("Process allocated to Memory Block %d of size %d\n", memory[bestFitIdx].id, processSize);
    } else {
