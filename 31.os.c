#include <stdio.h>

#define MAX_FRAMES 3
#define MAX_PAGES 10

int main() {
    int reference_string[MAX_PAGES] = {0, 1, 2, 3, 0, 1, 4, 0, 1, 2}; // Example reference string
    int page_frames[MAX_FRAMES] = {-1, -1, -1}; // -1 indicates an empty frame

    int page_faults = 0;
    int frame_index = 0; // Index to keep track of the oldest page in the frames

    printf("Reference String: ");
    for (int i = 0; i < MAX_PAGES; i++) {
        printf("%d ", reference_string[i]);
    }
    printf("\n");

    for (int i = 0; i < MAX_PAGES; i++) {
        int page = reference_string[i];
        int found = 0;

        // Check if the page is already in memory
        for (int j = 0; j < MAX_FRAMES; j++) {
            if (page_frames[j] == page) {
                found = 1;
                break;
            }
        }

        if (!found) {
            page_faults++;
            page_frames[frame_index] = page; // Replace the oldest page with the new page
            frame_index = (frame_index + 1) % MAX_FRAMES; // Update the oldest page index
        }

        printf("Page %d -> Page Frames: ", page);
        for (int j = 0; j < MAX_FRAMES; j++) {
            if (page_frames[j] == -1) {
                printf(" - ");
            } else {
                printf("%d ", page_frames[j]);
            }
        }
        printf("\n");
    }

    printf("Total Page Faults: %d\n", page_faults);

    return 0;
}
